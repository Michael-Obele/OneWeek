...
<div class="mx-auto max-w-7xl py-20">
	<section class="">
		<h2
			class="flex flex-wrap items-center justify-center gap-2 text-center text-4xl leading-normal font-semibold text-white max-md:flex-col md:text-5xl">
			Hiring Made Simple
		</h2>

		<p class="mx-auto mt-4 max-w-3xl text-center text-balance text-white/60 md:mt-6 md:text-lg">
			We've streamlined the hiring process to make it easy for you to get started. Here's how
			OneWeek works
		</p>
			...
			<div class="">
				<ul class="space-y-2">
					{#each items as item, index}
						<li class="text-right {selectedState !== index + 1 ? 'text-white/60' : ''}">
							<button
								class="mx-auto flex w-full items-center justify-end gap-5 max-md:w-10/12"
								onclick={() => (selectedState = index + 1)}>
								{item.title}
								<div
									class="grid h-12 w-12 shrink-0 place-items-center rounded-full border duration-100 {selectedState ===
									index + 1
